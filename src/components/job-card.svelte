...
>
  <a
    ...
    class="grid w-full grid-cols-[auto_fit-content(100%)] items-center gap-2 px-4 py-2"
  >
    <div class="grid grid-cols-[minmax(0,32px)_auto] items-center gap-2">
      <img width={32} height={32} alt={job.companyName} src={job?.companyLogo} class="rounded-full object-contain" />
        ...
        <div class="grid auto-cols-max grid-flow-col gap-2">
          {#if job?.requiredLocation}
            ...
            >
              ...
              />
              <span class="grid grid-cols-[fit-content(100%)_auto] rounded-full uppercase">{job.requiredLocation}</span>
            </div>
          {/if}
          {#if job?.salary}
            <div
            ...
            >
              <span class="grid grid-cols-[fit-content(100%)_auto] rounded-full uppercase">💸 {job.salary}</span>
            </div>
          {/if}
        </div>
      </div>
